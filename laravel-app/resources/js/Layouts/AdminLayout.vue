...
const navItems = [
  ...
  { name: 'Usuarios', href: route('admin.usuarios'), icon: '👥' },
]
...
<template>
  <div class="min-h-screen bg-gray-100 flex flex-col">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
          <div class="flex items-center gap-3">
            <button class="lg:hidden p-1 rounded hover:bg-blue-800" @click="sidebarOpen = !sidebarOpen">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/>
              </svg>
            </button>
            <Link :href="route('admin.dashboard')" class="flex items-center gap-2">
              <span class="text-2xl font-bold">PQRS</span>
              <span class="hidden sm:block text-blue-300 text-sm">Admin — UTS</span>
            </Link>
          </div>
          <div class="flex items-center gap-4">
            <span class="text-blue-200 text-sm hidden sm:block">{{ page.props.auth.user?.name }}</span>
            <button @click="logout" class="text-sm bg-blue-800 hover:bg-blue-700 px-3 py-1.5 rounded transition">
              Cerrar sesión
            </button>
          </div>
        </div>
    ...
    <div class="flex flex-1">
      ...
      <aside
        :class="[
          'bg-white shadow-md w-64 flex-shrink-0 lg:translate-x-0 lg:static',
          'fixed inset-y-0 left-0 z-10 pt-16 transition-transform duration-200',
      ...
      <div v-if="sidebarOpen" class="fixed inset-0 bg-black/30 z-0 lg:hidden" @click="sidebarOpen = false" />
